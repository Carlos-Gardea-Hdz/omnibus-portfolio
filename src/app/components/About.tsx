...
const TIMELINE = [
  {
    ...
    role: { es: 'Entorno Industrial', en: 'Industrial Environment' },
    color: '#6B7A99',
  },
  {
  ...
  },
  {
    ...
    role: { es: 'Arquitectura Avanzada', en: 'Advanced Architecture' },
    color: '#FF4500',
    current: true,
  },
];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
            ...
            <div className="relative w-full max-w-xs">
              <div className="aspect-square rounded-2xl border border-[#E2E8F0] dark:border-[#1E2330] bg-white dark:bg-[#111318] overflow-hidden">
                ...
                <div className="w-full h-full flex flex-col items-center justify-center"
                  style={{
                    background: 'radial-gradient(ellipse at 50% 30%, rgba(255,69,0,0.08) 0%, transparent 60%)',
                  }}
                >
                  <div className="w-24 h-24 rounded-full border-2 border-[#FF4500]/40 bg-[#FF4500]/10 flex items-center justify-center mb-4">
                    <span className="font-display text-[#FF4500] text-3xl">CG</span>
                  </div>
                  <span className="font-display text-[#0D1117] dark:text-[#F0F4FF] text-lg">Carlos Gardea</span>
                  <span className="font-code text-[#64748B] dark:text-[#6B7A99] text-xs mt-1">Software Engineer</span>
                  <div className="flex items-center gap-1.5 mt-3">
                    <MapPin size={11} className="text-[#6B7A99]" />
                    <span className="font-code text-[#6B7A99] text-[11px]">Ciudad Juárez, MX</span>
                  </div>
                </div>
            ...
            <div className="flex flex-col gap-3">
              {EDUCATION.map((edu, i) => (
                ...
                >
                  ...
                  <div>
                    <div className="font-body text-[#0D1117] dark:text-[#F0F4FF] text-sm">{edu.title}</div>
                    <div className="font-code text-[#64748B] dark:text-[#6B7A99] text-[11px]">{edu.subtitle}</div>
                  </div>
                </motion.div>
              ))}
            </div>
          ...
          >
            ...
            <div className="flex flex-wrap gap-2 mb-10">
              {['Laravel', 'PHP 8.5', 'React 19', 'TypeScript', 'Docker', 'PostgreSQL', 'Go', 'Rust', 'DevOps', 'DDD', 'CQRS'].map((skill) => (
                <span
                  key={skill}
                ...
                <div className="flex flex-col gap-6">
                  {TIMELINE.map((item, i) => (
                    ...
                    >
                      ...
                      <div className="pb-2">
                        <div className="font-display text-[#0D1117] dark:text-[#F0F4FF] text-base" style={{ color: item.current ? item.color : undefined }}>
                          {item.company}
                        </div>
                        <div className="font-body text-[#64748B] dark:text-[#6B7A99] text-sm">{item.role[lang]}</div>
                        <div className="font-code text-[#64748B] dark:text-[#6B7A99] text-[11px] mt-0.5">{item.period[lang]}</div>
                      </div>
                    </motion.div>
                  ))}
                </div>
